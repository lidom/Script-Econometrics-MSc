...
X_i\overset{iid}{\sim} F_X,
$$ 
where $F_X$ is a normal distribution with mean $(\theta=)\mu=10$ and variance $\sigma^2=5$. 

In the following, we want to use a Monte Carlo simulation to approximate the distribution of the estimator 
$$
\hat{X}_n =  \frac{1}{n}\sum_{i=1}^n X_i
$$
of $\mu=10.$ We consider two sample sizes $n=5$ and $n=50.$


::: {.callout-note}
Here we have specified the distribution $F_X$ completely by setting $F_X=\mathcal{N}(\mu=10,\sigma^2=5).$ This is such a simple case, that we can actually use mathematical derivations to derive the distribution of $\hat{X}_n.$ (Often, this is not possible.)
...
The following `R` code generates `B` many realizations of the random sample $X_i\overset{iid}{\sim}\mathcal{N}(\mu,\sigma^2)$
\begin{align*}
&(X_{1,1,obs},\dots,X_{n,1,obs})\\
&(X_{1,2,obs},\dots,X_{n,2,obs})\\
& \hspace{2cm}\vdots \\
...
#| label: fig-ecdf
#| fig-cap: "In black-solid: Empirical distribution functions $\\hat{F}_{X_{n},B}$ of $(B=10000)$ simulated realizations $\\bar{X}_{n,1,obs},\\dots,\\bar{X}_{n,B,obs}$ for $n=5,50.$ In blue-dashed: The theoretical distribution functions for $n=5,50.$ The empirical (simulation based) and the theoretical distribution functions match perfectly." 
library(scales)
par(mfrow=c(1,2))
plot(ecdf(estimates_mat[,1]), main="n=5", ylab="", xlab="", col = "black", xlim = range(estimates_mat[,1]))
mtext(expression(mu==10), side = 1, at = 10, line = 2.5)
curve(pnorm(x, mean=10, sd=sqrt(5/5)), add=T, col=alpha("blue", 0.25), lty = 3, lwd=4)
plot(ecdf(estimates_mat[,2]), main="n=50", ylab="", xlab="", col = "black",  xlim = range(estimates_mat[,1]))
mtext(expression(mu==10), side = 1, at = 10, line = 2.5)
...
#| fig-cap: "In black-solid: Empirical histrograms of $(B=10000)$ simulated realizations $\\bar{X}_{n,1,obs},\\dots,\\bar{X}_{n,B,obs}$ for $n=5,50.$ In blue-dashed: The theoretical density functions for $n=5,50.$ The empirical (simulation based) histrograms and the theoretical density functions match perfectly." 
library(scales)
par(mfrow=c(1,2))
...
hist(estimates_mat[,2], main="n=50", xlab="", xlim = range(estimates_mat[,1]), prob = TRUE, ylim = c(0, 1.2))
mtext(expression(mu==10), side = 1, at = 10, line = 2.5)

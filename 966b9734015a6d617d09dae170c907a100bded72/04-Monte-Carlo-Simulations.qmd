...
Any of the the above assessment metrics require us to compute **means**, i.e. $E()$, of random variables: 
...
We can use a Monte Carlo (MC) simulation to approximate these means by **sample means** using the **law of large numbers** (see @thm-SLLN1 in @sec-lsinf) which states that a sample mean over iid random variables is able to approximate the population mean of these random variables as the number of random variables to average over gets large.
...
**1. Step:** Use a (pseudo-)random number generator to draw a realizations of the random sample $\{X_1,\dots,X_n\}$ for a given distribution $F_X$ and a given sample size $n:$ 
\begin{align*}
...
**Repeat** the two steps for $j=1,\dots,B$ with $B$ being a large number (e.g. $B=10,000$) to generate $B$ independent realizations of $\hat\theta_n$
\begin{align*}
...
$$ 
which allow us to compute the following Monte Carlo (MC) approximations based on sample means over the generated $B$ realizations of $\hat\theta_n$:

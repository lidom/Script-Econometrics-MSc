...
- The log-log plot (@fig-loglog) has a slope of $-1$ which suggests that $\operatorname{MSE}(\bar{X}_n)$ is proportional to $n^{-1};$ i.e.
$$
\begin{align*}
...
\end{align*}
$$
where $\texttt{Constant}$ depends only on the data generating process and the estimator. 

...
<!-- $$
P\left(\{H_{1},H_2\}\right)=P\left(H_{1}\right)+P\left(H_{2}\right)=\frac{1}{2}+\frac{1}{2}=1
$$
$$
P\left(\{T_{1},T_2\}\right)=P\left(T_{1}\right)+P\left(T_{2}\right)=\frac{1}{2}+\frac{1}{2}=1
$$ -->
<!-- $$
P\left(H_{1}\right)=P\left(H_{2}\right)=P\left(T_{1}\right)=P\left(T_{2}\right)=1 / 2
=P\left(\left\{H_{1}, T_{2}\right\}\right)+P\left(\left\{T_{1}, H_{2}\right\}\right)+P\left(\left\{T_{1}, T_{2}\right\}\right)
$$  -->
<!-- and -->
$$
\begin{align*}
...
In other instances, we **derive** independence by verifying that the definition of independence $P(A B)=P(A)P(B)$ holds. For example, in tossing a fair die *once*, let $A=\{2,4,6\}$ be the event of observing an even number and let $B=\{1,2,3,4\}$ be the event of observing no $5$ and no $6$. Then, 
...
**Example:** Draw two cards from a deck, without replacement. Let $A$ be the event that the first draw is Ace of Clubs and let $B$ be the event that the second draw is Queen of Diamonds. Then 
\begin{align*}
P(A B) 
&=P(A) \;\;P(B \mid A)\\[2ex]
&=(1 / 52)\; (1 / 51)
...
#### Cumulative Distribution Function {#sec-Defcdf}
...
#### Marginal Distribution Functions and Marginal Density Functions 
...
contour(x1, x2, z, col="#4545FF", drawlabels=FALSE, nlevels=5,
        xlab=expression(x[2]), ylab=expression(x[1]),  lwd=1,xlim=c(-3,4),ylim=range(x2),frame.plot=FALSE,axes=FALSE,xaxs = "i", yaxs = "i")
...
#### Conditional Density Function {#sec-condDistr}
...
text(2, -2, labels=expression(x[2]==0.7))
...
#        title = "Marginal Densities")
legend("topleft", col="darkgreen", lty=2.1, lwd=.98, 
legend = expression(f(x[1]~ "|" ~ X[2]==0.7)), bty="n",
...
**Note:** Moments determine the tail of a distribution (but not much else); see @LB2000. Roughly: The more moments a distribution has the faster converge the tails to zero. Distributions with compact supports (e.g. the uniform distribution $\mathcal{U}[a,b]$) have infinitely many moments. The Normal distribution has also infinitely many moments, even though this distribution has not a compact support; see @sec-NormalDistr. 
...
The **Law of Total Expectation** or **Law of Iterated Expectation** states that 
$$
E\big(E(X_1|X_2,\ldots,X_d)\big)=E(X_1).
$$ 

To show this result, we, first compute the conditional mean of $X_1$ given $X_2,\dots,X_d$, and then mean with respect to $X_2,\dots,X_d.$
...
$$
Of course, the same statement applies to the marginal density of $Y$ given $X=x$ for all $x\in\mathbb{R}$. That is, $X$ and $Y$ are two independent real-valued random variables *if and only if* 
...
1. $X_{1}, \ldots, X_{n}$ are all **independent** from each other and 
2. $X_{1}, \ldots, X_{n}$ have **identical** marginal distributions, i.e., $X_i\sim F_X$ for all $i=1,\dots,n$. 
...
If 
$X_{1} \sim$ Binomial $\left(n_1, p\right)$ and 
$X_{2} \sim$ Binomial $\left(n_2, p\right)$ and if 
$X_1$ and $X_2$ 
...
#### The Normal (or Gaussian) Distribution {#sec-NormalDistr}
...
(i) If $X \sim \mathcal{N}\left(\mu, \sigma^{2}\right)$ then $Z=(X-\mu) / \sigma \sim \mathcal{N}(0,1)$

(ii) If $Z \sim \mathcal{N}(0,1)$ then $X=\mu+\sigma Z \sim \mathcal{N}\left(\mu, \sigma^{2}\right)$

(iii) If $X_{i} \sim \mathcal{N}\left(\mu_{i}, \sigma_{i}^{2}\right), i=1, \ldots, n$ are independent then
$$
...
#### The Student $t$ Distribution {#sec-tdist}
...
* [Exercises for Chapter 2](https://www.dropbox.com/scl/fi/id6hc7q13veovkopf6xnt/Ch2_Exercises1.pdf?rlkey=fhyv5m2fjw6p9kv2strkxomun&st=jiltpae4&dl=0)

* [Exercises for Chapter 2 with Solutions](https://www.dropbox.com/scl/fi/q4x4q4q996mav47sax8nf/Ch2_Exercises_with_Solutions1.pdf?rlkey=69osulookyvx96ttvj4ovo66a&st=03oydfn1&dl=0)

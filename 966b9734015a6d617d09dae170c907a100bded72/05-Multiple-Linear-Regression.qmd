...
$$
\begin{eqnarray*}
  Y_i&=&\underset{(1\times K)}{X_i^{\top}}\underset{(K\times 1)}{\beta} +\varepsilon_i, \quad i=1,\dots,n,
\end{eqnarray*}
$$
where 
$$
...
::: {.callout-caution}
Usually, we do not use a different notation for observed realizations 
$(Y_{i,obs},X_{i1,obs},\dots,X_{iK,obs})\in\mathbb{R}^{K+1}$ 
...
* This implies that the $(K\times K)$-dimensional matrix $X^{\top}X$ has full rank, i.e. that 
$$
\operatorname{rank}(X^{\top}X)=K\quad\text{(a.s.)}
$$

* Thus $(X^{\top}X)$ is invertible; i.e. there exists a $(K\times K)$-dimensional matrix $(X^{\top}X)^{-1}$ such that 
$$
(X^{\top}X)(X^{\top}X)^{-1} = (X^{\top}X)^{-1}(X^{\top}X) = I_K.
$$ 
:::
...
The error terms $\varepsilon_i$ are called homoskedastic if the conditional variance 
$$
...
If $E(\varepsilon_i|X_i)=0,$ then also the unconditional mean of the error term is zero, i.e.
$$
E(\varepsilon_i|X_i)=0 \quad\Rightarrow\quad E(\varepsilon_i)=0.
$$
:::
::: 
...
If $E(\varepsilon_i|X_{i})=0,$ then the regressors and the error term are orthogonal to each other, i.e,
$$
E(\varepsilon_i|X_i)=0 \quad\Rightarrow\quad E(X_{ik}\varepsilon_i)=0
$$
for all $k=1,\dots,K.$
:::
:::
...
Because the mean of the error term is zero ($E(\varepsilon_i)=0$; see @thm-a), it follows that the orthogonality property, $E(X_{ik}\varepsilon_i)=0,$ is equivalent to a zero correlation property. 
...
\begin{eqnarray*}
E(\varepsilon_i|X_i)=0 \quad\Rightarrow\quad Cov(\varepsilon_i,X_{ik})&=&0\quad\text{for all}\quad k=1,\dots,K.
\end{eqnarray*}
:::
::: 
...
::: {.callout-tip icon="false"}
## Correlations between the error term $\epsilon_i$ and regressors $X_{ik}$

By @thm-c, we have that

$$
\underbrace{E(\epsilon_i|X_i)=0}_{A} \Rightarrow \underbrace{\operatorname{Cov}(\epsilon_i,X_{ik})=0\quad \text{for all}\quad k=1,\dots,K}_{B}
$$
...
$$
\underbrace{\operatorname{Cov}(\epsilon_i,X_{ik})\neq 0\;\text{for at least one}\; k=1,\dots,K}_{\neg B} \Rightarrow \underbrace{E(\epsilon_i|X_i)\neq 0}_{\neg A}
$$
I.e., if any of the regressors $X_{ik}$, $k=1,\dots,K,$ has a non-zero covariance with the error term $\epsilon_i,$ then the exogeneity assumption is violated. 
:::
...
$$
S_n(b)=\sum_{i=1}^n\big(\underbrace{Y_i-X_i^{\top}b}_{\text{$i$th residual}}\big)^2
$$ 
...
$$
\begin{aligned}
\underset{(K\times 1)}{\frac{\partial S(b)}{\partial b}} &=-2\left(X^{\top}Y -X^{\top} Xb\right).
\end{aligned}
$$
...
\Leftrightarrow\qquad X^{\top} X\hat{\beta}
&=\underset{(K\times 1)}{X^{\top}Y}.
\end{align*}
$${#eq-NormalEquations}
From our rank assumption (Assumption 3) it follows that $X^{\top}X$ is an invertible $(K\times K)$-dimensional matrix which allows us to solve the equation system in @eq-NormalEquations by 
$$
\begin{aligned}
\underset{(K\times 1)}{\hat{\beta}} &=\left(X^{\top} X\right)^{-1} X^{\top} Y.
\end{aligned}
$$
...
E(X_i(\overbrace{Y_i - X_i^{\top}\beta}^{=\varepsilon_i})) &= 0\\[2ex]
%\Leftrightarrow \hspace{1.5cm}
E(X_iY_i) - E(X_iX_i^{\top})\beta & = 0\\[2ex]
E(X_iX_i^{\top})\beta & =  E(X_iY_i)  \\[2ex]
\beta & = \left(E(X_iX_i^{\top})\right)^{-1} E(X_iY_i) 
\end{align*}
$$
...
\begin{align*}
\hat\beta_{mm} 
& = \left(\frac{1}{n}\sum_{i=1}^n X_iX_i^{\top}\right)^{-1} \frac{1}{n}\sum_{i=1}^n X_iY_i,
\end{align*}
$${#eq-MMEstimator}
where 
$$
\begin{align*}
E(X_iX_i^{\top}) & \quad \text{is substituted by}\quad \frac{1}{n}\sum_{i=1}^n X_iX_i^{\top}\\[2ex]
...
$$
\beta \neq \tilde \beta = \left(E(X_iX_i^{\top})\right)^{-1} E(X_iY_i).
$$



The OLS derivation of the OLS estimator does not require a check whether $\beta$ is actually identified. By the law of large numbers (sample means converge in probability to population means; see @sec-lsinf), the OLS estimator 
$$
\begin{align*}
\hat\beta 
& = \left(X^{\top}X\right)^{-1} X^{\top}Y\\[2ex]
& = \underbrace{\left(\frac{1}{n}\sum_{i=1}^n X_iX_i^{\top}\right)^{-1}}_{\approx (E(X_iX_i^{\top}))^{-1}} \; \underbrace{\frac{1}{n}\sum_{i=1}^n X_iY_i}_{\approx E(X_iY_i)} 
\end{align*}
$$
is a consistent estimator of $\tilde\beta,$ 
$$
\begin{align*}
\hat\beta \to_p  \tilde\beta = (E(X_iX_i^{\top}))^{-1} E(X_iY_i) \quad \text{as}\quad n\to\infty. 
\end{align*}
$$

That is, the OLS estimator will estimate the parameter of interest $\beta$ only if  
$$
\beta = \tilde \beta = \left(E(X_iX_i^{\top})\right)^{-1} E(X_iY_i);
$$
...
$$
P_X=P_X^{\top}\quad\text{ and }\quad M_X=M_X^{\top}
$$
...
(c) Moreover, we have that 
     * $X^{\top}P_X=X^{\top}$ and $P_X X=X$
     * $X^{\top}M_X=0$ and $M_XX=0$ 
     * $P_XM_X=0$ and $M_XP_X=0$ 

The properties (a)-(c) follow directly from the definitions of $P_X$ and $M_X$ (check it out).  

Using properties (a)-(c), one can show that the residual vector $\hat\varepsilon=(\hat\varepsilon_1,\dots,\hat\varepsilon_n)^{\top}$ is orthogonal to each of the column vectors in $X$, i.e 
$$
\begin{eqnarray}
X^{\top}\hat\varepsilon&=&X^{\top}M_XY\quad{\small\text{(By Def. of $M_X$)}}\\[2ex]
\Leftrightarrow\;\; X^{\top}\hat\varepsilon&=&\underset{(K\times n)}{0}\underset{(n\times 1)}{Y}\quad{\small\text{(since $X^{\top}M_X=0$)}}\\[2ex]
\Leftrightarrow\;\; X^{\top}\hat\varepsilon&=&\underset{(K\times 1)}{0}.
\end{eqnarray}
...
Moreover, the equation $X^{\top}\hat\varepsilon=0$ implies also that the residual vector $\hat{\varepsilon}$ is orthogonal to the predicted values vector, since
$$
\begin{align*}
X^{\top}\hat\varepsilon&=0\\
\Rightarrow\;\hat\beta^{\top}X^{\top}\hat\varepsilon&=\hat\beta^{\top}0\\
\Leftrightarrow\;\hat Y^{\top}\hat\varepsilon&=0.
\end{align*}
$$
...
$$
X^{\top}\hat\varepsilon=\underset{(K\times 1)}{0}
$$ 
...
**Variance decomposition:** A further useful result that can be shown using the properties of $P_X$ and $M_X$ is that $Y^{\top}Y=\hat{Y}^{\top}\hat{Y}+\hat\varepsilon^{\top}\hat\varepsilon$, i.e.
\begin{eqnarray*}
Y^{\top}Y&=&(\hat Y+\hat\varepsilon)^{\top}(\hat Y+\hat\varepsilon)\notag\\
  &=&(P_XY+M_XY)^{\top}(P_XY+M_XY)\notag\\
  &=&(Y^{\top}P_X^{\top}+Y^{\top}M_X^{\top})(P_XY+M_XY)\notag\\
  &=&Y^{\top}P_X^{\top}P_XY+Y^{\top}M_X^{\top}M_XY+0\notag\\
  &=&\hat{Y}^{\top}\hat{Y}+\hat\varepsilon^{\top}\hat\varepsilon
\end{eqnarray*}
The decomposition 
$$
\hat{Y}^{\top}\hat{Y}+\hat\varepsilon^{\top}\hat\varepsilon
...
\end{eqnarray}
where $\bar{Y}=\frac{1}{n}\sum_{i=1}^nY_i$ and $\bar{\hat{Y}}=\frac{1}{n}\sum_{i=1}^n\hat{Y}_i,$ and where $\bar{Y}=\bar{\hat{Y}}.$ 
:::
::: 

::: {.proof}
From equation $X^{\top}\hat\varepsilon=0$ we have for regressions with intercept that $\sum_{i=1}^n\hat\varepsilon_i=0$. Hence, from $Y_i=\hat{Y}_i+\hat\varepsilon_i$ it follows that
\begin{eqnarray*}
  \frac{1}{n}\sum_{i=1}^n Y_i&=&\frac{1}{n}\sum_{i=1}^n \hat{Y}_i+\frac{1}{n}\sum_{i=1}^n \hat\varepsilon_i\\
  \bar{Y}&=&\bar{\hat{Y}}+0
\end{eqnarray*}

Using the decomposition $Y^{\top}Y=\hat{Y}^{\top}\hat{Y}+\hat\varepsilon^{\top}\hat\varepsilon$, we can now derive the result:
\begin{eqnarray*}
   Y^{\top}Y&=&\hat{Y}^{\top}\hat{Y}+\hat\varepsilon^{\top}\hat\varepsilon\\
   Y^{\top}Y-n\bar{Y}^2&=&\hat{Y}^{\top}\hat{Y}-n\bar{Y}^2+\hat\varepsilon^{\top}\hat\varepsilon\\
   Y^{\top}Y-n\bar{Y}^2&=&\hat{Y}^{\top}\hat{Y}-n\bar{\hat{Y}}^2+\hat\varepsilon^{\top}\hat\varepsilon\quad\text{(by $\bar{Y}=\bar{\hat{Y}}$)}\\
...
\end{align*}
$$
These properties apply to the OLS estimator 
$$
\hat\beta=f(Y)=(X^{\top}X)^{-1}X^{\top}Y
$$ 
since
\begin{align*}
f(Y_1+Y_2) 
& = (X^{\top}X)^{-1}X^{\top}(Y_1+Y_2)\\
& = (X^{\top}X)^{-1}X^{\top}Y_1 + (X^{\top}X)^{-1}X^{\top}Y_2\\
& = f(Y_1) + f(Y_2)
\end{align*}
and 
\begin{align*}
...
&= E((X^{\top}X)^{-1}X^{\top}\underbrace{Y}_{=X\beta+\varepsilon}|X) - \beta \\[2ex]
&= E((X^{\top}X)^{-1}X^{\top}(X\beta+\varepsilon)|X)                 - \beta \\[2ex]
&= E(\underbrace{(X^{\top}X)^{-1}X^{\top}X}_{=I_K}\beta|X) + E((X^{\top}X)^{-1}X^{\top}\varepsilon|X) - \beta \\[2ex]
&= \underbrace{E(\beta|X)}_{=\beta} + \underbrace{E((X^{\top}X)^{-1}X^{\top}\varepsilon|X)}_{=(X^{\top}X)^{-1}X^{\top}E(\varepsilon|X)} - \beta \\[2ex]
&=  (X^{\top}X)^{-1}X^{\top}\underbrace{E(\varepsilon|X)}_{=0} =\underset{(K\times 1)}{0}  
\end{align*}
$$
...
&=Var((X^{\top}X)^{-1}X^{\top}\varepsilon|X),
\end{align*}
$$
where we used that 
$$
...
&=\beta+(X^{\top}X)^{-1}X^{\top}\varepsilon\\[2ex]
\Leftrightarrow \highlight{\hat\beta - \beta} 
& = \highlight{(X^{\top}X)^{-1}X^{\top}\varepsilon}.
\end{align*}
$$
...
\begin{align*}
Var(Z|X)
&=\underbrace{E\Big[\overbrace{(Z-E(Z|X))}^{(K\times 1)}\overbrace{(Z-E(Z|X))^{\top}}^{(1\times K)}|X\Big]}_{K\times K}\\[2ex]
&=\underbrace{E\big[ZZ^{\top}|X\big]}_{K\times K} - \underbrace{E\big[Z|X\big]E\big[Z^{\top}|X\big]}_{K\times K}
\end{align*}
$$
:::
:::

Using the definition of the conditional variance (@def-CondVarMult) for multivariate random variables $(Z=(X^{\top}X)^{-1}X^{\top}\varepsilon)$ we have that 
$$
\begin{align*}
&Var(\hat\beta|X)=\\[2ex]
%&=Var(\hat\beta - \beta|X)\\[2ex]
&=Var((X^{\top}X)^{-1}X^{\top}\varepsilon|X)\\[2ex]
&=E\Big[\big((X^{\top}X)^{-1}X^{\top}\varepsilon-\underbrace{E((X^{\top}X)^{-1}X^{\top}\varepsilon|X)}_{=0}\big)\times\\[2ex]
&\phantom{=\Big(}\,\times\big((X^{\top}X)^{-1}X^{\top}\varepsilon-\underbrace{E((X^{\top}X)^{-1}X^{\top}\varepsilon|X)}_{=0}\big)^{\top}|X\Big]\\[2ex]
&=E\left[((X^{\top}X)^{-1}X^{\top}\varepsilon)((X^{\top}X)^{-1}X^{\top}\varepsilon)^{\top}|X\right]\\[2ex]
&=E\left[(X^{\top}X)^{-1}X^{\top}\varepsilon\varepsilon^{\top} X(X^{\top}X)^{-1}|X\right]\\[2ex]
&=\;\;\;(X^{\top}X)^{-1}X^{\top}\underbrace{E\left(\varepsilon\varepsilon^{\top}|X\right)}_{=Var(\varepsilon|X)}X(X^{\top}X)^{-1}
\end{align*}
$$
...
\end{matrix}\right)
= \sigma^2 I_n,
%& = E\left(\varepsilon\varepsilon^{\top}|X\right) - E\left(\varepsilon|X\right) E\left(\varepsilon^{\top}|X\right)\\[2ex]
%& = E\left(\varepsilon\varepsilon^{\top}|X\right) - \underset{(K\times K)}{0}\\[2ex]
\end{align*}
$$
...
\begin{align*}
Var(\hat\beta|X)
&=(X^{\top}X)^{-1}X^{\top} \left(\sigma^2I_n\right)X(X^{\top}X)^{-1}\\[2ex]
&=\sigma^2(X^{\top}X)^{-1}X^{\top}X(X^{\top}X)^{-1}\\[2ex] 
&=\sigma^2(X^{\top}X)^{-1}.
\end{align*}
$$
...
$$
\underset{(K\times 1)}{\hat\beta}=(X^{\top}X)^{-1}X^{\top}Y
$$
...
$$
\tilde{\beta}=\Big(C\;\;\overbrace{-\left(X^{\top}X\right)^{-1}X^{\top}+\left(X^{\top}X\right)^{-1}X^{\top}\;}^{=0}\;\Big)Y.
$$
Let now $D=C-\left(X^{\top}X\right)^{-1}X^{\top}$, then
$$
\begin{align*}
\tilde{\beta}&=\left(D+\left(X^{\top}X\right)^{-1}X^{\top}\right)Y\\
\tilde{\beta}&=DY + \left(X^{\top}X\right)^{-1}X^{\top}Y\\
\tilde{\beta}&=D\left(X{\beta}+{\varepsilon}\right) + \left(X^{\top}X\right)^{-1}X^{\top}Y
\end{align*}
$$
Thus 
$$
...
\tilde{\beta}-{\beta}&=D{\varepsilon} + \highlight{\left(X^{\top}X\right)^{-1}X^{\top}{\varepsilon}}
\end{align*}
$$
such that 
$$
\tilde{\beta}-{\beta}=\left(D + \left(X^{\top}X\right)^{-1}X^{\top}\right){\varepsilon},
$${#eq-c3e18}
where we used that 
$$
\begin{align*}
\highlight{\hat\beta-\beta}&=(X^{\top}X)^{-1}X^{\top}Y-\beta\\[2ex]
&=(X^{\top}X)^{-1}X^{\top}(X\beta+\varepsilon)-\beta\\[2ex]
&=\highlight{(X^{\top}X)^{-1}X^{\top}\varepsilon}.
\end{align*}
$$
...
\begin{align*}
Var(\tilde{\beta}|X) 
&= Var((D + (X^{\top}X)^{-1}X^{\top}){\varepsilon}|X)\\[2ex]
&= (D + (X^{\top}X)^{-1}X^{\top})Var({\varepsilon}|X)(D^{\top} + X(X^{\top}X)^{-1})\\[2ex]
&= \sigma^2(D + (X^{\top}X)^{-1}X^{\top})I_n(D^{\top} + X(X^{\top}X)^{-1})\\[2ex]
&= \sigma^2\left(DD^{\top}+(X^{\top}X)^{-1}\right)\quad \text{(using that $DX=0$)} \\[2ex]
&\geq\sigma^2(X^{\top}X)^{-1} \quad \text{(using that $DD^{\top}\geq 0$)}\\[2ex]
&= Var(\hat{\beta}|X).
\end{align*}
$$
Finally, we need to show that $DD^{\top}$ is really positive semidefinite (i.e. $DD^{\top}\geq 0$ in matrix sense):
$$
\begin{align*}
a^{\top}DD^{\top}a=(D^{\top}a)^{\top}(D^{\top}a)=\tilde{a}^{\top}\tilde{a}=\sum_{k=1}^K \tilde{a}^2_k \geq 0,
\end{align*}
$$
...
<!-- Remember: -->
<!-- \begin{itemize} -->
<!-- \item $(A+\hat{\beta})^{\top}=A^{\top}+\hat{\beta}^{\top}$ -->
<!-- \item $(AB)^{\top}=\hat{\beta}^{\top}A^{\top}$ -->
<!-- \item $A^{\top} = A$ $\Leftrightarrow$ $A$ is a symmetric matrix -->
<!-- \end{itemize} -->
:::
...
That is, we constructed a $(n\times K)$-dimensional matrix $X$ for which Assumption 3 $(\operatorname{rank}(X)=K)$ is violated. Therefore $(X^{\top}X)$ does not have full rank and is thus not invertible. 
::: 

The estimation result is not computable since $(X^{\top}X)$ is not invertible due to the perfect multicollinearity between the intercept and the two dummy variables `X_female` and `X_male`, i.e. </br> 
<center>
`X_1` = `X_female` $+$ `X_male` 
</center>
</br>
...
* The diagnostic plot indicates that the variance increases with $\hat{Y}_i=X_i^{\top}\hat{\beta}.$

::: {.callout-note}
Plotting against the fitted values is actually a quite smart idea since this also works for multiple predictors $X_{i1},\dots,X_{iK}$ with $K\geq 3$. 
:::
...
However, in order to understand the statistical properties of the estimator $\hat{\beta}$ we need to view it as a random variable which yields different realizations in repeated realizations of the random sample from the Model (@eq-LinMod). 
...
* [Exercises for Chapter 5](https://www.dropbox.com/scl/fi/xxk62qh53yk11omgyengv/Ch5_Exercises1.pdf?rlkey=odrz6y0zpjel489trhf95pl7r&st=gyyhl96i&dl=0)


* [Exercises of Chapter 5 with Solutions](https://www.dropbox.com/scl/fi/8ndrotu3pwkiqapnm5mwg/Ch5_Exercises_with_Solutions1.pdf?rlkey=24j0xuhqoz830wj2xg3ut7qd2&st=0o597u2z&dl=0) 

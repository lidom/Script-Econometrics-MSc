...
\end{array}
\end{equation*}
In case of a two-sided $t$-test, we reject the null hypothesis if the observed realization of the $t$-test, $t_{obs}$, is "far away" from zero either by being sufficiently smaller or greater than $r$. 

The corresponding two-sided critical values are denoted by 

* $c_{\alpha/2}<0$ and 
* $c_{1-\alpha/2}>0$, 

where $c_{1-\alpha/2}>0$ is the $(1-\alpha/2)$ quantile of the $t$-distribution with $(n-K)$ degrees of freedom, and where 
$$
c_{1-\alpha/2}=-c_{\alpha/2}
$$ 
due to the symmetry of the $t$-distribution. 

These critical values define the following rejection and the non-rejection regions 
\begin{align*}
...
\end{array}
\end{equation*}
In case of a one-sided $t$-test, we will reject the null if $t_{obs}$ is sufficiently "far away" from zero in the relevant direction of $H_1$. The corresponding critical value is either 

* $c_{\alpha}$ (when $H_1:\beta_k< r$) 

or 

* $c_{1-\alpha}$ (when $H_1:\beta_k> r$), 

where $c_{\alpha}$ is the $\alpha$ quantile of the $t$-distribution with $(n-K)$ degrees of freedom, and where $c_{1-\alpha}=-c_{\alpha}$ 
due to the symmetry of the $t$-distribution. 


This leads to the following rejection and the non-rejection regions:


For $H_0: \beta_k=0\quad$ versus $\quad H_1: \beta_k < 0$:
\begin{align*}
...
& = 1 - P(\text{Not reject H$_0$ given H$_1$ is true})\\[2ex]
& = P(\text{Reject H$_0$ given H$_1$ is true})\\[2ex]
& = P(\text{Detect a violation of the null})
\end{align*}


Unfortunately, computing the power of a statistical test is usually impossible, since this would require knowing the distribution of the test statistic under the alternative hypothesis H$_1.$

We can, however derive the power of the $t$ test for some given scenario. 
...
::: {.callout-tip}
Of course, usually we do not know the standard error of the estimator, but have to estimate it. 

But it is true that the standard error of the OLS estimator $\hat{\beta}_k$ is proportional to $1/\sqrt{n}.$
:::
...
If H$_0$ is true with $\beta_k=\beta_k^{(0)},$ then
\begin{align*}
t
...
If H$_1$ is true with $\beta_k-\beta_k^{(0)}>0,$ then 
\begin{align*}
...
$$
and where $z_{1-\alpha}$ denotes the $(1-\alpha)$ quantile of the standard normal distribution $\mathcal{N}(0,1).$  
...
The following Graphic illustrates the probability of a type II error and the power for the case where 
...
such that 
$$
\frac{\sqrt{n}(\beta_k-\beta_k^{(0)})}{4.5} = 2.
$$
```{r, fig.align="center", echo=FALSE, fig.width = 8, fig.height = 5, out.width = "1\\textwidth"}

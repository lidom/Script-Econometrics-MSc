...
* the realizations of $(R\hat\beta_n -r^{(0)})|X$ will scatter around the *unknown* $(R\beta -r^{(0)})$ in a Gaussian fashion. 
* if $H_0$ is correct (i.e., if $R\beta-r^{(0)}=0$), the realizations of $(R\hat\beta_n-r^{(0)})|X$ scatter around the $(q\times 1)$ vector $0$.  
...
Generally, we can only derive the distribution of $T$ under $H_0;$ i.e. under the scenario that $H_0$ is true. For instance,

* in @sec-testmultp we showed that $F\overset{H_0}{\sim} F_{(q,n-K)}.$

* in @sec-testingsinglep we showed that $T\overset{H_0}{\sim} t_{(n-K)}$

Typically, we do **not know** the distribution of a test statistic under $H_1.$



### Test Decisions and Decisions Errors 
...
* We **can reject** $H_0:$ </br>if the observed value $T_{\text{obs}}$ is surprisingly extreme (i.e. very large or very small depending on $H_1$) in comparison to the distribution of $T$ under $H_0.$ 
<!-- </br>(Extreme: in direction of $H_1;$ i.e. very large or very small in comparision to the null distribution of $T.$)  -->

* We **cannot reject** $H_0:$ </br>if the observed value $T_{\text{obs}}$ is in the range of typical $T$ realizations under $H_0.$
...
::: {.callout-tip}
# Size 
The probability of a type-I-error event is also called **size** of the test statistic $T,$ 
\begin{align*}
...
\end{align*}
Since we want to be sure that we correctly reject $H_0,$ we want test statistics for which we can control the size (e.g. size$\;<\alpha=0.05$). 
:::
...
A statistical hypothesis test, $T,$ is a **valid test** if its size (probability of a type-I-error) can be bounded from above by the **significance level (nominal size)** $\alpha$, i.e. if  
\begin{align*}
...
$$
\text{Size}=P(\text{type-I-error})=\alpha.
$$
...
* **rejection regions/critical value,** 
...
#### Rejection Regions / Critical Values of the $F$-Test {-}
...
$$
F_{obs}\in \{\text{rejection region}\}=\; ]c_{1-\alpha},\infty[
$$
* We cannot rejection $H_0$ if
$$
F_{obs}\not \in \{\text{rejection region}\}=\; ]c_{1-\alpha},\infty[
$$


This allows us to show that the $F$-test is an exact test under Assumptions 1-4$^\ast$:
\begin{align*}
\text{Size}=&P(\text{reject } H_0 \;\;|\;\; H_0\text{ is true})\\[2ex]
=&P(F \in \{\text{rejection region}\} \;\;|\;\; H_0\text{ is true})\\[2ex]
...
Absence of evidence (against $H_0:\beta_k=0$) is not evidence of absence (i.e. is no evidence of $H_0:\beta_k=0$).
::: 
...
* We can rejection $H_0$ if
$$
T_{obs}\in \{\text{rejection region}\}=\;]-\infty,c_{\alpha/2}[\;\;\cup\;\;]c_{1-\alpha/2}, \infty[
$$
* We cannot rejection $H_0$ if
$$
T_{obs}\not \in \{\text{rejection region}\}=\;]-\infty,c_{\alpha/2}[\;\;\cup\;\;]c_{1-\alpha/2}, \infty[
$$

This allows us to show that the $t$-test is an exact test under Assumptions 1-4$^\ast$:
\begin{align*}
...
&=1-0.36=0.64
\end{align*}
That is, we detect the  violation of the null hypothesis $(\beta_k - \beta_k^{(0)}=3)$ with probability $0.64;$ i.e., we expect to detect the violation of the null hypothesis in $64\%$ of resamplings from the random sample. 
...
\end{align*}
where $c_{1-\alpha/2}$ denotes the $(1-\alpha)$ quantile of the $t$-distribution with $(n-K)$ degrees of freedom. Next, we can do the following equivalent transformations
\begin{align*}
P\left(-c_{1-\alpha/2}\leq\frac{\hat\beta_k-\beta_k}{\widehat{\operatorname{SE}}(\hat\beta_k|X)}\leq c_{1-\alpha/2}\right)&=1-\alpha\\
...
Here, we simulated data "under the null hypothesis" and thus it is not surpising that we cannot reject the null hypothesis at a significance level of, for instance, $\alpha=0.05$. 

However, in repeated samples we should nevertheless observe $\alpha\cdot 100\%$ type I errors (false rejections of $H_0$) under the null hypothesis. Let's check the type-I-error rate using the following Monte Carlo simulation:
```{r}

...
* We **can reject** $H_0,$ </br>if the observed value $T_{\text{obs}}$ is in the range of extreme (i.e. very large or very small depending on $H_1$) $T$ realizations under $H_0.$ 
...
\end{align*}
Since we want to be sure that we correctly reject $H_0,$ we want test statistics for which we can control the probability of a type-I-error event (i.e. the size) from above by some chosen level; e.g. 

...
**Caution:** A random $X$ requires us to consider conditional means and variances given $X.$ That is, if we were able to resample from the population (from the data-generating process), we would do so by fixing (conditioning on) the observed $X.$ 
...
$$
is called the ***simple* linear regression model**.
:::
...
::: {.callout-note}
Together with the random sample assumption (Assumption 1, Part (b)) @eq-assExogen implies even **strict exogeneity** 
$$
E(\varepsilon|X) = \underset{(n\times 1)}{0}
$$ 
since we have independence across $i=1,\dots,n$.  Under strict exogeneity, the mean of the random vector $\varepsilon$ is zero irrespective of the realizations of $X_1,\dots,X_n$. 
:::
...
and 
\begin{align*}
&Var\left(\varepsilon|X\right)=\\[2ex] 
& = \left(\begin{matrix}
...
Thus, here one assumes that, for a given realization of $X$, the error process is 
   * **uncorrelated**, i.e. $Cov(\varepsilon_i,\varepsilon_j|X)=0$ for all $i\neq j$ and 
   * **homoskedastic**, i.e. $Var(\varepsilon_i|X)=\sigma^2$ for all $i.$
...
$$ 
are equal to the constant $\sigma^2>0$ for all $i=1,\dots,n$ and for every possible realization of $X_i$. For doing small sample inference (see @sec-ssinf), we will assume that the error terms are homoskedastik. 
...
The properties (a)-(c) follow directly from the definitions of $P_X$ and $M_X$ (check it out).  Using these properties one can show that the residual vector $\hat\varepsilon=(\hat\varepsilon_1,\dots,\hat\varepsilon_n)'$ is orthogonal to each of the column vectors in $X$, i.e 
\begin{eqnarray}
...
Another insight from equation $X'\hat\varepsilon=0$ is that the vector $\hat\varepsilon$ has to satisfy $K$ linear restrictions which means it looses $K$ degrees of freedom. Consequently, the vector of residuals $\hat\varepsilon$ has only $n-K$ so-called *degrees of freedom*.  This loss of $K$ degrees of freedom also appears in the definition of the *unbiased* variance estimator
\begin{align}
...
## Method of Moments Estimator {#sec-MMEstimator}
...
**Showing linearity:** A function $f(Y)$ is called linear if for and $Y_1$ and $Y_2$ such that $Y=Y_1+Y_2$ and for any $a\in\mathbb{R}$ 
\begin{align*}
...
&=  (X'X)^{-1}X'\underbrace{E(\varepsilon|X)}_{=0} =0.  
\end{align*}
$$
...
$$
Var(Z|X)=\underbrace{E\Big[\overbrace{(Z-E(Z|X))}^{(K\times 1)}\overbrace{(Z-E(Z|X))'}^{(1\times K)}|X\Big]}_{K\times K}.
$$ 
:::
:::
...
\end{align*}
where $I_n$ is the $(n\times n)$ dimensional identity matrix with ones on the diagonal and zeros everywhere else. Thus under the assumption of spherical errors, we have that
$$
\begin{align*}
Var(\hat\beta|X)
...
If it hold true that 

1. $Y = X\beta + \varepsilon$
2. $\operatorname{rank}(X)=K$ a.s.
3. $E(\varepsilon|X) = \underset{(n\times 1)}{0}$
...
::: {.callout-tip}
A $(K\times K)$ matrix $D$ is called "positive semidefinite" if $a'Da\geq 0$ for any $K$-dimensional vector $a\in\mathbb{R}^K$.  
:::
::: 
:::
...
::: {.callout-important}
Above in our theoretical part, we have not mentioned the possibility of categorical, i.e., factor variable. 

* How can one compute the estimators from non-numeric, verbal factor levels like `female` and `male`? 

Well, we need to transform such non-numeric data in numeric data. How this is done, is described in the following. 
:::

We use data provided in the accompanying materials of Stock and Watson's *Introduction to Econometrics* textbook [@stock2015]. You can download the data stored as an xlsx-file `cps_ch3.xlsx` [HERE](https://github.com/lidom/Script-Econometrics-MSc/blob/main/85467d963958861b842aa28b9eb262b1ea250101/data/cps_ch3.xlsx). 
...
<font color="#FF0000">An error message!</font>  🤬 We stepped into the **dummy variable trap**!
...
<!-- * [Exercises for Chapter 3](Exercises/Ch3_Exercises.pdf) -->

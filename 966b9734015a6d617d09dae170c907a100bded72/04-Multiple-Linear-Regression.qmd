...
Further, way more extensive material on linear algebra can be found, for instance: 
...
* This implies that the $(K\times K)$ matrix $X'X$ has full rank, i.e. that 
$$
\operatorname{rank}(X'X)=K\quad(a.s.)
$$

* Thus $(X'X)$ is invertible; i.e. there exists a $(K\times K)$ matrix $(X'X)^{-1}$ such that 
$$
...
Thus, here one assumes that, for all possible realizations of $X$, the error process is 
   * **uncorrelated:** $Cov(\varepsilon_i,\varepsilon_j|X)=0$ for all $i\neq j$ and 
   * **homoskedastic:** $Var(\varepsilon_i|X)=\sigma^2$ for all $i.$

...
$$
where $\varepsilon=(\varepsilon_1,\dots,\varepsilon_n)^{\top},$ and where $\mathcal{N}_n\left(0,\sigma^2I_n\right)$ denotes the $n$-dimensional normal distribution with $(n\times 1)$-dimensional mean zero vector $0$ and $(n\times n)$-dimensional covariance matrix $\sigma^2I_n.$ 
...
where 
$$
Var(\hat\beta_n|X)=\underset{(K\times K)}{\sigma^2(X^{\top}X)^{-1}}.
$$
:::
::: 
...
\end{align*} 
$$
and because $(X^{\top}X)^{-1}X^{\top}\varepsilon$ is just a linear combination of the normally distributed error terms $\varepsilon$ which, therefore, is again normally distributed, conditionally on $X$. Note that the specific normal distribution depends on the observed realization of $X$. 
:::
...
* the $(q \times K)$ matrix $R,$ which describes the considered linear combinations of the unknown $\beta=(\beta_1,\dots,\beta_K)^{\top}$ values, and 
* the $(q\times 1)$ vector $r^{(0)}=(r^{(0)}_{1},\dots,r^{(0)}_{q})^{\top}$ of null hypothetical values
...
$$
\begin{align*}
(R\hat\beta_n-r^{(0)})|X&\sim\mathcal{N}_q\left(R\beta -r^{(0)}, RVar(\hat\beta_n|X)R^{\top}\right)%\\[2ex]
%\Leftrightarrow\quad \left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\hat\beta_n-r^{(0)})|X&\sim\\[2ex]
%\hspace{2cm}\sim\mathcal{N}_q\left(\left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\beta -r^{(0)}),I_q\right)
\end{align*}
$$ 
...
$$
\begin{align*}
(R\hat\beta_n-r^{(0)})|X&\overset{{\color{red}{H_0}}}{\sim}\mathcal{N}_q\left(\,{\color{red}0}\,,RVar(\hat\beta_n|X)R^{\top}\right)\\[2ex]
\Leftrightarrow\quad \left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\hat\beta_n-r^{(0)})\highlight{|X}&\overset{{\color{red}{H_0}}}{\sim}\underbrace{\mathcal{N}_q\left(\,{\color{red}0}\,,I_q\right)}_{\highlight{\text{doesn't depend on $X$}}}\\[2ex]
\highlight{\Rightarrow}\quad \left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\hat\beta_n-r^{(0)})&\overset{{\color{red}{H_0}}}{\sim}\mathcal{N}_q\left(\,{\color{red}0}\,,I_q\right),
\end{align*}
$$ 
...
$$
\left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\hat\beta_n-r^{(0)})
$$ 
...
\begin{align*}
W 
& = \overbrace{\left(\left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\hat\beta_n-r^{(0)})\right)^{\top}\;\;}^{(1\times q)}\\
&\;\;\;\;\;\;\underbrace{\left(\left(RVar(\hat\beta_n|X)R^{\top}\right)^{-1/2}(R\hat\beta_n-r^{(0)})\right)}_{(q \times 1)}\\[2ex]
& =\underbrace{(R\hat\beta_n -r^{(0)})^{\top}}_{(1\times q)}\underbrace{[RVar(\hat\beta_n|X)R^{\top}]^{-1}}_{(q\times q)}\underbrace{(R\hat\beta_n -r^{(0)})}_{(q\times 1)}
\end{align*}
$${#eq-TestStatW}
...
Usually, however, we do not know $Var(\hat\beta_n|X)=\sigma^2 (X^{\top}X)^{-1},$ since we usually do not know the value of $\sigma^2.$ Thus, we need to substitute $\sigma^2$ by an estimator. 

For exact finite sample inference, we need a variance estimator of $\sigma^2$ for which we can derive its exact small sample distribution. Therefore, we require Assumption 4$^*$ of spherical errors, i.e. $Var(\varepsilon|X)=\sigma^2I_n,$ which implies that $Var(\hat\beta_n|X)=\sigma^2(X^{\top}X)^{-1}$, and where $\sigma^2$ can be estimated by the unbiased ($UB$) variance estimator  
$$
...
$$
Var(\hat\beta_n|X)=\sigma^2 (X^{\top}X)^{-1}
$$ 
in @eq-TestStatW by its estimator 
$$
\widehat{Var}(\hat\beta_n|X)=s_{UB}^2 (X^{\top}X)^{-1}
$$ 
leads to the $F$-test statistic
$$
F=(R\hat\beta_n -r^{(0)})^{\top}[R(s_{UB}^2(X^{\top}X)^{-1})R^{\top}]^{-1}(R\hat\beta_n -r^{(0)})/q
$$
...
where 
$$
\widehat{Var}(\hat{\beta}_k|X)=s^2_{UB}[(X^{\top}X)^{-1}]_{(k,k)},
$$ 
and where $[(X^{\top}X)^{-1}]_{(k,k)}$ denotes the element in the $k$th row and $k$th column of the $(K\times K)$ matrix $(X^{\top}X)^{-1}.$
...
$$
where $0<\alpha<1$ denotes the significance level and $c_{\alpha}$ denotes the $\alpha$-quantile of the $t$-distribution with $(n-K)$ degrees of freedom. 
...
* If $\Theta_j,$ $j=0,1,$ contains only **one value** 
$$
\Theta_j=\theta^{(j)},
$$ 
it is called a **simple hypothesis**. 
* If $\Theta_j,$ $j=0,1,$ contains **multiple values**, it is called a **composite hypothesis**.
...
<!-- ::: {.callout-important}
**Caution:** Retaining $H_0$ does not mean that we have evidence that $H_0$ is true. In fact, in real data, basically every $H_0$ is false. Retaining $H_0$ means that we do not have sufficient evidence against $H_0.$
::: -->
...
::: {.callout-caution}
Not being able to reject $H_0,$ does not mean a validation/confirmation of $H_0,$ but only reflects that we do not have sufficient evidence against a possibly false $H_0.$ Indeed, a given violation of $H_0$ may only be too small to stand out from the estimation errors; i.e. we may do a type-II-error. 

Problem is, we generally cannot control the probability of type-II-errors since we do not know the distribution of the test statistic under $H_1.$ We can only control the probability of type-I-errors since we know the distribution of the test statistic under $H_0.$

Therefore, if you are not able to reject $H_0,$ **never ever** state something like: "I conclude $H_0$ is true." (In fact, in real data, basically every $H_0$ is false.)
...
A statistical hypothesis test procedure is called **valid** if its size can be bounded from above by the chosen **significance level (nominal size)** $\alpha$, i.e. if  
$$
\begin{align*}
...
Generally, of course, the value of $\alpha$ is arbitrary and you can also choose $\alpha=0.00231.$

::: {.callout-note}
# Exact vs Conservative vs Invalid Testing
...
Since we want to detect given violations of $H_0,$ we want test statistics with large power.
...
**Caution:** Consistency of a statistical test is an important property. However, this property implies that even small, practically irrelevant violations of $H_0$, i.e., $|\theta-\theta_0|\approx 0,$ will be detected if the sample size is sufficiently large. 
...
* $z_{1-\alpha}$ denotes the $(1-\alpha)$-quantile of the standard normal distribution $\mathcal{N}(0,1),$  
...
$$ 
You can provide your own values for the sample size $n$ and for the parameter vector $\beta=(\beta_1,\beta_2,\beta_3)^{\top}$. 
```{r}
...
* [Exercises for Chapter 6](https://www.dropbox.com/scl/fi/hyrokd4720da8iubigmvd/Ch6_Exercises1.pdf?rlkey=2yw1px0j7fp4criz3ol8qzbas&st=cj791hzi&dl=0)

* [Exercises of Chapter 6 with Solutions](https://www.dropbox.com/scl/fi/7ml9kkam4e7j72tyh3awh/Ch6_Exercises_with_Solutions1.pdf?rlkey=7wyxelycpaf072oe2j1w0745m&st=x1ojtu8y&dl=0) 

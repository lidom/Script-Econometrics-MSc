...
\Leftrightarrow\quad \left(RVar(\hat\beta_n|X)R'\right)^{-1/2}(R\hat\beta_n-r^{(0)})|X&\sim\underbrace{\mathcal{N}_q\left(R\beta -r^{(0)},I_q\right)}_{\highlight{\text{doesn't depend on $X$}}}\\[2ex]
\highlight{\Rightarrow}\quad \left(RVar(\hat\beta_n|X)R'\right)^{-1/2}(R\hat\beta_n-r^{(0)}) &\sim\mathcal{N}_q\left(R\beta -r^{(0)},I_q\right),
\end{align*}
$$ 
...
\Leftrightarrow\quad \left(RVar(\hat\beta_n|X)R'\right)^{-1/2}(R\hat\beta_n-r^{(0)})|X&\overset{{\color{red}{H_0}}}{\sim}\underbrace{\mathcal{N}_q\left(\,{\color{red}0}\,,I_q\right)}_{\highlight{\text{doesn't depend on $X$}}}\\[2ex]
...
\end{equation*}
$$
For instance, in case of the $F$-test, $f_T$ is the $F$-distribution and in case of the $t$-test, $f_T$ is the $t$-distribution. 

However, the distribution of $T$ under $H_1$ is typically unknown. 
$$
\begin{equation*}
\begin{array}{ll}
T & \overset{{H_1}}\sim ❓ 
...
$$
of the random test statistic $T$ to decide: 
...
Consider, without loss of generality, the following right-sided, one-sided hypothesis:
$$
\begin{align*}
&H_0: \beta_k \leq \beta_k^{(0)}\\
\text{versus}\quad 
...
\end{align*}
$$
(This without loss of generality, since effectively the same arguments apply to the case of the left-sided version.)

The case of a one-sided hypothesis is slightly more involved since the null hypothesis is a **compound hypothesis**: 
$$
...
$$ 
for conducting the $t$-test (as presented in @sec-TDecRS), we know that, under $H_0,$  
$$
\begin{align*}
T(\beta_k^{(0)})
&=
...
&=
\underbrace{\frac{\hat\beta_k - \beta_k}{\widehat{SE}(\hat\beta_{k}|X)}}_{\sim t_{(n-K)}} + 
\underbrace{\frac{\beta_k - \beta_k^{(0)}}{\widehat{SE}(\hat\beta_{k}|X)}}_{{\color{red}{\leq 0}}}\\[2ex]
&\leq 
\frac{\hat\beta_k - \beta_k}{\widehat{SE}(\hat\beta_{k}|X)} \overset{H_0}{\sim} t_{(n-K)},
\end{align*}
...
Equivalent arguments, can be applied for to the one-sided $t$-test for the left-sided version of the $t$-test.
...
### Example: Power of the One-Sided $t$-Test
...
### Interpreting the $p$-Value {#sec-pValue}


The $p$-value of a test-statistic $T$ is defined as the probability of observing realizations of $T$ that are equal to or larger than the observed value $T_{\text{obs}}$ **given that the null hypothesis is true**
$$
p_{\text{obs}}=P(T\geq T_{\text{obs}}\;|\;H_0 \text{ is true})
$$

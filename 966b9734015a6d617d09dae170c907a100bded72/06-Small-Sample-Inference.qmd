...
$$
\frac{\hat\beta_{n,k}-\beta_k}{\widehat{\operatorname{SE}}(\hat\beta_k|X)}\sim t_{(n-K)}
$${#eq-CIDistr}
Therefore,
$$
\begin{align*}
...
\Leftrightarrow P\left(\hat\beta_{n,k}-c_{n,1-\alpha/2}\widehat{\operatorname{SE}}(\hat\beta_{n,k}|X)\leq \beta_k\leq\hat\beta_k +c_{n,1-\alpha/2}\widehat{\operatorname{SE}}(\hat\beta_{n,k}|X)\right)&=1-\alpha\\[2ex]
\Leftrightarrow P\left(\beta_k\in\underbrace{\left[\hat\beta_{n,k}-c_{n,1-\alpha/2}\widehat{\operatorname{SE}}(\hat\beta_{n,k}|X),\;\hat\beta_k +c_{n,1-\alpha/2}\widehat{\operatorname{SE}}(\hat\beta_{n,k}|X)\right]}_{=:\operatorname{CI}_{k,n,1-\alpha}}\right)&=1-\alpha
\end{align*}
$$
...
Unfortunately, this "frequentist" interpretation is not a statement about a single given $\operatorname{CI}_{k,n,1-\alpha}$ realization computed for a given data set. A given, realized $\operatorname{CI}_{k,n,1-\alpha}$ will either contain the true parameter $\beta_k$ or not, and usually we do not know the answer. So, confidence intervals are quite hard to interpret. However, they are very well suited as a tool to visualize estimation uncertainties in different parameter estimators, for instance, across $\hat\beta_k$, $k=1,\dots,K$.   
...
Generally, we can only derive the distribution of $T$ under $H_0;$ i.e. under the scenario that $H_0$ is true, 
$$
\begin{equation*}
\begin{array}{ll}
...
\end{equation*}
$$
For instance, in case of the $F$-test, $f_{T_n}$ is the $F$-distribution and in case of the $t$-test, $f_{T_n}$ is the $t$-distribution. 
...
* We **reject** $H_0$ if 
$$
T_{n,\text{obs}}\in \mathcal{R}_{n,\alpha} 
$$ 
where $\mathcal{R}_{n,\alpha} $ denotes the **rejection region**; i.e. a range of $T$ values that we see only very rarely under $H_0.$ Note that the rejection region depends on the sample size $n$ and the chosen significance level $\alpha.$ 

* We **cannot reject** $H_0$ 
$$
T_{n,\text{obs}}\not \in \mathcal{R}_{n,\alpha}
$$ 

(Equivalent descisions are made based on the $p$-value.)
...
::: {.callout-caution}
Not being able to reject $H_0,$ does not mean a confirmation of $H_0.$ 
...
$$
\begin{align*}
P(T_n\in\mathcal{R}_{n,\alpha}|\beta_k < \beta_k^{(0)}) \highlight{\leq}  P(T_n\in\mathcal{R}_{n,\alpha} \;|\;\beta_k = \beta_k^{(0)}) & = \alpha\\[2ex]
\Leftrightarrow\quad  
...
**Caution:** This implies that even small, practically irrelevant violations of $H_0$ will be detected with a large probability when $n\to\infty.$
...
* Let $X$ be deterministic such that $T_n|X \overset{d}{=}T_n$ and 
...
Of course, usually we do not know the standard error of the estimator, but have to estimate it. However, it is true that the standard error of the OLS estimator $\hat{\beta}_{n,k}$ is **proportional to** $1/\sqrt{n},$
$$
\operatorname{SE}(\hat\beta_{n,k}|X) = \frac{1}{\sqrt{n}}\cdot\texttt{constant},
$$
where the $\texttt{constant}$ does not depend on $n.$
:::

Under this simplified setup and under Assumptions 1-4^$\ast,$ the $t$-test statistic is **normally distributed**. 

That is, if $H_0$ is true with $\beta_k=\beta_k^{(0)},$ then
$$
\begin{align*}
T_n
&=\frac{\hat\beta_{n,k}-\beta_k^{(0)}}{\frac{1}{\sqrt{n}}4.5}\\[2ex]
&=\frac{\sqrt{n}(\hat\beta_{n,k}-\beta_k)}{4.5} \overset{H_0}{\sim} \mathcal{N}(0,1).
\end{align*}
$$
...
Let's consider, without loss of generality, the $p$-value for a left-sided version of a one-sided hypothesis test:
$$
\begin{align*}
p_{\text{left},\,\text{obs}} 
...
\end{align*}
$$
where $F_{T_n}$ denotes the cumulative distribution function (see @sec-Defcdf) of $T_n.$ 
...
Under $H_0,$ the $p$-value is a **uniformly distributed random variable** 
$$
...
Observe that $p_{\text{left}}$ is defined as a transformed random variable; namely by transforming the random variable $T_n$ using the cumulative distribution function $F_{T_n},$ where, under $H_0,$
$$
...
If the claim is correct, then the random variable $p_{\text{left}}$ has, under $H_0,$ the cumulative distribution function of $\mathcal{U}[0,1];$ i.e.
$$

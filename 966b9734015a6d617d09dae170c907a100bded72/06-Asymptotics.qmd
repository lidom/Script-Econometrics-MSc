...
1. Often you will see statements like $Z_n\to_{d} N(0,1)$ or $Z_n\overset{a}{\sim}N(0,1)$, which should be read as $\lim_{n\to\infty}F_n(x) = \Phi(x)$ for all $x\in\mathbb{R},$ where $\Phi$ denotes the  distribution function of the standard normal distribution. 
...
- If $X_n'X_n\to_{p} \Sigma_{X'X}$ then $(X_n'X_n)^{-1}\to_{p} \Sigma_{X'X}^{-1}$, provided $\Sigma_{X'X}$ is a nonsingular/invertible matrix.
...
* If $X_n\to_{d} X\quad$ and $\quad Y_n\to_{p} \alpha,\quad$  then $\quad X_n+Y_n\to_{d} X + \alpha$

* If $X_n\to_{d} X\quad$ and $\quad Y_n\to_{p} 0,\quad$ then $\quad X_n'Y_n\to_{p} 0$

* If $X_n\to_{d} X\quad$ and $\quad A_n\to_{p} A,\quad$ then $\quad A_n X_n\to_{d} AX$, where it is assumed that $A_n$ and $X_n$ are "conformable" (i.e., the matrix- and vector-dimensions fit to each other). 
:::
:::
...
\sqrt{n}(\bar{Z}_n-\mu)
$$ 
converges to a **multivariate** normal distribution as $n\to\infty$, we need to check whether the **uni**variate stochastic sequence $\{\lambda'Z_i\}$ is i.i.d. with $E(\lambda'Z_i)=\lambda'\mu$ and $Var(\lambda'Z_i)=\lambda'\Sigma\lambda$ for any  $\lambda\in\mathbb{R}^K$. 
...
\begin{align*}
\Sigma_{X'X}
&=E\left(S_{X'X}\right)=E(n^{-1}X'X)\\[2ex]
&=E\left(n^{-1}\sum_{i=1}^nX_iX_i'\right) = E\left(X_iX_i'\right).
\end{align*}
For the below results, we need to restate the full rank assumption (Assumption 3 of @sec-MLR) with respect to $\Sigma_{X'X}.$

**Assumption 3$^\ast$: Rank Condition (for Asymptotics)** </br>
The $(K\times K)$ matrix 
$$
\Sigma_{X'X}=E(S_{X'X})
$$
...
By contrast to the multiple parameter tests for small samples (see @sec-testmultp), we can work here with a heterosedasticity robust test statistic which is applicable for heteroskedastic error terms:
$$
\begin{align*}

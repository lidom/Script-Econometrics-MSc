...
* with $E(\lambda'Z_i)=\lambda'\mu$ for all $i=1,\dots,n$ and 
* $Var(\lambda'Z_i)=\lambda'\Sigma\lambda$ for all $i=1,\dots,n$
 

The latter is the case if the multivariate stochastic sequence $\{Z_i\}$ is an i.i.d. sequence with $E(Z_i)=\mu$ and $Var(Z_i)=\Sigma.$
...
where 
$$
\sigma^2=\lim_{n\to\infty}Var(\sqrt{n}(\hat\theta_n-\theta))
$$ 
is called the asymptotic variance. 

Due to the $\sqrt{n}$-scaling, $\theta_n$ is also called $\sqrt{n}$-consistent. 
:::
:::
...
$$ 
then we have shown that $\hat\theta_n$ converges to zero in the mean square sense 
$$

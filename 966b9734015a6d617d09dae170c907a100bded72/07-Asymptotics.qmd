...
@thm-OLSnormality1 can also be stated (and proofed) for conditionally heteroskedastic error terms. In this case one gets for large sample sizes ($n\to\infty$),
$$
\begin{align*}
...
$$
\hat\beta_{n,k}\to_d\mathcal{N}\left(\beta_k, \;n^{-1}\;\left[\Sigma_{X'X}^{-1}E(\varepsilon^2_iX_iX_i')\Sigma_{X'X}^{-1}\right]_{(k,k)}\right)
$$
as $n\to\infty.$ 

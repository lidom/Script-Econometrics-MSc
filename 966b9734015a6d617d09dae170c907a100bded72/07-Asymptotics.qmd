...
Suppose $\{Z_n\}$ is a stochastic sequence of random scalars, vectors, or matrices and that $f$ is a *continuous* function that does not depended on $n$. Then
$$
\begin{align*}
Z_n\to_{p}  \alpha \quad & \Rightarrow\quad f(Z_n)\to_{p} f(\alpha),\\[2ex]
Z_n\to_{as} \alpha \quad & \Rightarrow\quad f(Z_n)\to_{as} f(\alpha),\quad\text{and}\\[2ex]
Z_n\to_{d}  \alpha \quad & \Rightarrow\quad f(Z_n)\to_{d} f(\alpha),
\end{align*}
$$
where $\alpha\in\mathbb{R}$ is deterministic.
:::
:::
...
@thm-OLSnormality1 can also be stated (and proofed) for conditionally heteroskedastic error terms. In this case one gets
$$
...
$$
\begin{align*}
T&=\frac{\sqrt{n}\left(\hat{\beta}_k-\beta_k^{(0)}\right)}{\sqrt{\left[S_{X'X}^{-1}\widehat{E}(\varepsilon^2_iX_iX_i')S^{-1}_{X'X}\right]_{(k,k)}}}\\[1.5ex] 
\text{where}\qquad T&\overset{H_0}{\to}_d\mathcal{N}(0,1),\quad\text{as}\quad n\to\infty,
\end{align*}
and where $\left[S_{X'X}^{-1}\widehat{E}(\varepsilon^2_iX_iX_i')S^{-1}_{X'X}\right]_{(k,k)}$ denotes the element in the $k$th row and $k$th column of the $K\times K$ matrix $S_{X'X}^{-1}\widehat{E}(\varepsilon^2_iX_iX_i')S^{-1}_{X'X}.$
$$

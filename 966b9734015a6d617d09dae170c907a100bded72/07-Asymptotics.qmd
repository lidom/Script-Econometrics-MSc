...
1. Convergence in probability of a sequence of random vectors or matrices $\{Z_n\}$ to a deterministic vector or matrix $\alpha$ requires **element-wise** convergence in probability.
2. The above definition can also be stated for random limits; i.e., for $\alpha$ being a random variable. 
:::
:::
...
A sequence of univariate real valued random variables $\{Z_n\}$ **converges almost surely** to a deterministic $\alpha\in\mathbb{R}$ if
\begin{eqnarray*}
...
1. Almost sure convergence of a sequence of random vectors (or matrices) $\{Z_n\}$ to a deterministic vector (or matrix) $\alpha$ requires **element-wise** almost sure convergence. 
2. The above definition can also be stated for random limits; i.e., for $\alpha$ being a random variable.
:::
:::
...
1. the **mean** of the stochastic sequence $E(Z_n)$ must converge to the limit $\alpha,$ and 
2. the **variance** of the stochastic sequence $E(Z_n)$ must converge to zero. 
...
1. Mean square convergence of a sequence of random vectors (or matrices) $\{Z_n\}$ to a deterministic vector (or matrix) $\alpha$ requires *element-wise* mean square convergence. 
2. The above definition can also be stated for random limits; i.e., for $\alpha$ being a random variable.
:::
:::
...
1. Often you will see statements like 
$$
...
By contrast to all other modes of convergence, @def-conv_distr only addresses the univariate case $(Z_n\in\mathbb{R}).$ The reason for this is that @def-conv_distr cannot simply applied element-wise since this would ignore the possible dependencies between the different uni-variate random variables elements of a random vector. 
...
The Cramér-Wold theorem (@thm-CramerWold) is needed since element-wise convergence in distribution generally does not imply convergence of the *joint* distribution of $Z_n$ to the *joint* distribution of $Z$; except, if all elements in the random vectors $Z_n$ and $Z$ are independent from each other.
...
- If $\frac{1}{n}\sum_{i=1}^nX_iX_i^{\top}=\frac{1}{n}X_n^{\top}X_n\to_{p} \Sigma_{X^{\top}X},$ then 
$$
\left(\frac{1}{n}X_n^{\top}X_n\right)^{-1}\to_{p} \Sigma_{X^{\top}X}^{-1},
$$ 
provided $\Sigma_{X^{\top}X}$ is a nonsingular/invertible matrix.
...
**Remark:** Sometimes, only the first two points in @thm-Slutsky are called "Slutsky's theorem."


::: {.callout icon=false}
#
::: {#exm-Slutsky}
## Most important special case of @thm-Slutsky

</br>


Let $\{X_n\}$ be a sequence of real valued $(K\times 1)$-dimensional random vectors such that 
$$
X_n\to_{d}\mathcal{N}_K(0,\Sigma)\quad\text{as}\quad n\to\infty,
$$ 
and let $\{A_n\}$ be a sequence of real valued $(L\times K)$-dimensional random matrices (or vectors if $L=1$) such that 
$$
A_n\to_{p} A \quad\text{as}\quad n\to\infty,
$$
where $A$ is a $(L\times K)$-dimensional deterministic matrix. Then, by @thm-Slutsky, $\{A_nX_n\}$ converges in distribution to an $L$-dimensional normal distribution,
$$
A_nX_n\to_{d}\mathcal{N}_L(0,A\Sigma A^{\top}).
$$
:::
:::
...
Using the Cramér-Wold device (@thm-CramerWold), the Lindeberg-Levy CLT (@thm-CLT1) can also be applied to $K$-dimensional estimators 
$$
...
(a) **i.i.d.:** The **uni**variate stochastic sequence $\{\lambda^{\top}Z_i\}$ **is i.i.d.** with
(b) **Mean:** $E(\lambda^{\top}Z_i)=\lambda^{\top}\mu$ for all $i=1,\dots,n,$ where $E(Z_i)=\mu\in\mathbb{R}^K$ denotes the $K$ dimensional mean vector, and 
(c) **Variance:** $Var(\lambda^{\top}Z_i)=\lambda^{\top}\Sigma\lambda$ for all $i=1,\dots,n,$ where $Var(Z_i)=\Sigma$ denotes the $(K\times K)$ dimensional variance-covariance matrix. 
...
#
::: {#def-ANorm}
## Asymptotic Normality, Asymptotic Variance, $\sqrt{n}$-Consistent 
...
As already discussed in @sec-EstimTheory, the MSE of $\hat\theta_n$ can be decomposed into the squared bias of $\hat\theta_n$ and the variance of $\hat\theta_n:$
$$
\begin{align*}
...
\end{align*}
2. The variance of the estimator converges to zero
$$
...
Using asymptotic statistics, allows us to drop the unrealistic normality and spherical errors assumption (Assumption 4$^\ast$) of @sec-ssinf, but still use our inference tools ($t$-tests, $F$-tests) from @sec-ssinf; as long as the sample size $n$ is "large." 
...
\end{align*}
$$
For the below results, we need to restate the full rank assumption (Assumption 3 of @sec-MLR) with respect to $\Sigma_{X^{\top}X}.$

**Assumption 3$^\ast$: (Population) Rank Condition** </br>
The $(K\times K)$ matrix 
$$
\Sigma_{X^{\top}X}=E(S_{X^{\top}X})
$$
<!-- $$ -->
<!-- n^{-1}X^{\top}X=S_{X^{\top}X}\to_{p}\Sigma_{X^{\top}X}\quad\text{as}\quad n\to\infty -->
<!-- $$ -->
has full rank $K$. I.e., $\Sigma_{X^{\top}X}$ is nonsingular and invertible. 
<!-- (Note, this assumption does not assume that $S_{X^{\top}X}\to_{p}\Sigma_{X^{\top}X}$, but if this convergence hold, it assumes that the limiting matrix is of full rank $K$.) -->
...
Under Assumptions 1, 2, 3$^\ast$, and 4, and under the assumption that $X_i$ has finite second moments, and $\varepsilon_i$ has finite first moments, we have that
$$
...
Under Assumption 1, 2, 3$^\ast$, and 4, and under the assumption that $X_i$ and $\varepsilon_i$ have finite second moments, and under the simplifying assumption of spherical errors 
$$
...
@thm-OLSnormality1 implies that for spherical errors and for largish sample sizes $n,$ the OLS estimator $\beta_n$ is **approximately** normal distributed, i.e.
$$
\begin{align*}
...
>  **Note:** Consistency of $S_{X^{\top}X}^{-1}$ is provided by @thm-Sxx1 and consistency of $s_{ub}^2$ is provided by @thm-Consistency_s1.
...
@thm-OLSnormality1 can also be stated (and proofed) for conditionally heteroskedastic error terms. In this case, $\beta_n$ is for large sample sizes $n$ **approximately** normal distributed, 
$$
\begin{align*}
...
$$ 
is here a placeholder for one of the existing **Heteroskedasticity Consistent (HC)** estimators of $E(\varepsilon^2X_iX_i^{\top})$:
...
General idea of the HC2-HC4 estimators is to increase the estimated variance in order to account for the effects of influential outliers. The residuals $\hat\varepsilon_i$ belonging to $X_i$ values that have a large leverage $h_i$ receive a higher weight and thus increase the value of $\widehat{E}(\varepsilon^2_iX_iX_i^{\top}).$ This strategy takes into account increased estimation uncertainties due to single influential outliers. <!-- (Too small variance estimates lead to false inference, too large variance estimates lead to conservative inference.) -->
:::
...
\end{align*}
$$
where the $(q \times K)$ matrix $R$ and the $q$-vector $r=(r_{1},\dots,r_{q})^{\top}$ are chosen by the statistician to specify her/his null hypothesis about the unknown true parameter vector $\beta$. To make sure that there are no redundant equations, it is required that $\operatorname{rank}(R)=q$.

By contrast to the multiple parameter tests for small samples (see @sec-testmultp), we can work here with a **heteroskedasticity robust** test statistic which is applicable for heteroskedastic error terms:
$$
\begin{align*}
...
\end{equation*}
$$
Selecting the $k$th diagonal element of the test-statistic in @eq-Ftestasymp and taking the square root yields 
$$
\begin{align*}
...
$$ 
denotes the element in the $k$th row and $k$th column of the $K\times K$ matrix $S_{X^{\top}X}^{-1}\widehat{E}(\varepsilon^2_iX_iX_i^{\top})S^{-1}_{X^{\top}X}.$ This $t$-test statistic allows for **heteroskedastic** error terms. 
...
> $$ 
where $[S_{X^{\top}X}^{-1}]_{(k,k)}$ denotes the element in the $k$th row and $k$th column of the $K\times K$ matrix $S_{X^{\top}X}^{-1}.$
...
Following the derivations in Chapter @sec-CIsmallsample, but using the expression for the robust standard errors, we get the following heteroskedasticity robust (random) $(1-\alpha)\cdot 100\%$ confidence interval 
$$
\operatorname{CI}^{\text{rob}}_{k,n,1-\alpha}=
\left[\hat\beta_k\pm t_{1-\alpha/2,n-K}\sqrt{n^{-1}[S_{X^{\top}X}^{-1}\widehat{E}(\varepsilon^2_iX_iX_i^{\top})S^{-1}_{X^{\top}X}]_{(k,k)}}\right].
$$
Here, the coverage probability is an asymptotic coverage probability with 
$$
P(\beta_k\in\operatorname{CI}^{\text{rob}}_{k,n,1-\alpha})\to 1-\alpha\quad\text{as}\quad n\to\infty.
$$
...
By contrast to our simulations in Chapter @sec-PSSI, we consider here a **non-Gaussian** and **heteroskedastic** error term
$$
...
> \end{align*}
> where the last steps follows from applying the variance formula for uniform random variables. 
The following `R`-function `myDataGenerator()` allows us to generate data from the above described data generating processes:
```{r}
...
* [Exercises for Chapter 7](https://www.dropbox.com/preview/Teaching/My_Courses/EC_MA/EC_MA_WS_25_26/Exercises/output/Ch7_Exercises1.pdf?session_id=e23e3b15c710460b)

* [Exercises of Chapter 7 with Solutions](https://www.dropbox.com/scl/fi/lukwgwqhpat56svnknh8t/Ch7_Exercises_with_Solutions1.pdf?rlkey=9i7ueeiqrlewcwo1617kvfhjr&st=mcpflw0i&dl=0) 

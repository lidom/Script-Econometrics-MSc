...
* the **uni**variate stochastic sequence $\{\lambda'Z_i\}$ is i.i.d. with
...
\end{align*}
$$
For the below results, we need to restate the full rank assumption (Assumption 3 of @sec-MLR) with respect to $\Sigma_{X'X}.$
...
>  **Note:** Consistency of $S_{X'X}^{-1}$ is provided by @thm-Sxx1 and consistency of $s_{ub}^2$ is provided by @thm-Consistency_s1.
...
General idea of the HC2-HC4 estimators is to increase the estimated variance in order to account for the effects of influential outliers. The residuals $\hat\varepsilon_i$ belonging to $X_i$ values that have a large leverage $h_i$ receive a higher weight and thus increase the value of $\widehat{E}(\varepsilon^2_iX_iX_i').$ This strategy takes into account increased estimation uncertainties due to single influential outliers. <!-- (Too small variance estimates lead to false inference, too large variance estimates lead to conservative inference.) -->
:::
...
$$ 
denotes the element in the $k$th row and $k$th column of the $K\times K$ matrix $S_{X'X}^{-1}\widehat{E}(\varepsilon^2_iX_iX_i')S^{-1}_{X'X}.$
...
Great! The nonparametric density estimation (estimated via `density()`) computed from the `5000` simulated realizations of $\hat\beta_2$ is indicating that $\hat\beta_2$ is really normally distributed as described by our theoretical results. 
 ...
 <!-- in @thm-OLSnormality1 (homoskedastic case) and in Equation @eq-OLSnormality1Rob (heterosceda# car::linearHypothesis(model = lm_obj, 
...
* [Exercises for Chapter 7](https://www.dropbox.com/scl/fi/q3j4dj3vptmdfc8zuea7p/Ch7_Exercises1.pdf?rlkey=wu9f1viv9d6xmqvkeiku91j6s&dl=0)

* [Exercises of Chapter 7 with Solutions](https://www.dropbox.com/scl/fi/xm9eceev8397tekzq6xf0/Ch7_Exercises_with_Solutions1.pdf?rlkey=ridhqkloeyd2nhurpgipgeexy&dl=0)

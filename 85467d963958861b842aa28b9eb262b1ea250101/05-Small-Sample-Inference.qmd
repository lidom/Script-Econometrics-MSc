...
### Check: Normally Distributed $\hat\beta|X$
...
In order to check the effect of "conditioning on $X$", let us use two different Monte Carlo simulations:
...
Then we plot both distributions side by side:
```{r}
set.seed(123)
...
* The estimated conditional distribution of $\hat{\beta}_2$ does match the theoretical conditional distribution of $\hat{\beta}_2$ given $X$
* The estimated **un**conditional distribution of $\hat{\beta}_2$ does not match the theoretical conditional distribution of $\hat{\beta}_2$  given $X$
...
* First, it means that we correctly control for the type I error rate since the type I error rate is not larger than the chosen significance level $\alpha$. 
* Second, it means that the test is not conservative (i.e. very efficient) since the empirical type I error rate is really close to the chosen significance level $\alpha$. (In fact, if we would increase the number of Monte Carlo repetitions, the empirical type I error rate would converge to the selected significance level $\alpha$ due to the law of large numbers.)
...
length(rejections)/B
```
Indeed, we can now reject the (here false) null hypothesis in approximately $39\%$ of all Monte Carlo replications. 

**Caution:** This also means that we are not able to "see" the violation of the null hypothesis in $100\%-39\%=62\%$ of cases. Therefore, we can never use an insignificant test result (p-value$\geq\alpha$) as a justification to accept the null hypothesis. Obviously, there are Type II error events, but since we typically do not know the distribution of the test statistic under the alternative hypothesis, we cannot control the Type II error rate. We can only control the Type I error rate by using a small significance level $\alpha$. 
...
```
We can use these two-sided confidence intervals to do hypothesis tests. This property of confidence intervals is called the duality of confidence intervals and hypothesis tests. 

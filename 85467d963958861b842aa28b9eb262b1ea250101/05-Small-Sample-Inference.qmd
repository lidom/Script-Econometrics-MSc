...
$$
Note that the distribution of $W$ conditional on $X$ does not depend on $X$; i.e. $W|X\overset{H_0}{\sim}\chi^2_{(q)}$, but $\chi^2_{(q)}$ does not depend on $X$, thus we can write $W\overset{H_0}{\sim} \chi^2_{(q)}.$
...
$${#eq-Ftest}
where $F_{(q,n-K)}$ denotes the $F$-distribution with $q$ numerator and $n-K$ denominator degrees of freedom. As in the case of $W$, the distribution of $F$ conditional on $X$ does not depend on $X$; i.e. $F|X\overset{H_0}{\sim}F_{(q,n-K)}$, but $F_{(q,n-K)}$ does not depend on $X$, thus we can write $F\overset{H_0}{\sim}F_{(q,n-K)}.$

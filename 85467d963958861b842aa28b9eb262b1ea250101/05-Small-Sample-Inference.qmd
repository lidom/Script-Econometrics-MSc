...
* The Monte Carlo approximation to the true variance of $\hat{\beta}_2|X$ is `var(hatbeta2_sim_cond)` $=$ `r round(var(hatbeta2_sim_cond),3)` and thus essentially equal to the theoretical counterpart.
* The Monte Carlo approximation to the true variance of $\hat{\beta}_2$ is `var(hatbeta2_sim_uncond)` $=$ `r round(var(hatbeta2_sim_uncond),3)` and thus larger than the theoretical counterpart.

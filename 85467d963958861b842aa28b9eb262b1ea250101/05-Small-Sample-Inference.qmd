...
* The theoretical variance of $\hat{\beta}_2|X$ is $\sigma^2[(X'X)^{-1}]_{22}=$ `var_true_beta_2` $=$ `r var_true_beta_2`.
* The Monte Carlo approximation to the true variance of $\hat{\beta}_2|X$ is `var(hatbeta2_sim_cond)` $=$ `r var(hatbeta2_sim_cond)`
* The Monte Carlo approximation to the true variance of $\hat{\beta}_2$ is `var(hatbeta2_sim_uncond)` $=$ `r var(hatbeta2_sim_uncond)`

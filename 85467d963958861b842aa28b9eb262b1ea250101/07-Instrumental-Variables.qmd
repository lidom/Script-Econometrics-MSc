...
[^1]: The instrumental variables $Z_{i2}$ correlate with $Y_{i1}$ only indirectly through $X_{i}'=(X_{i1}',Y_{i2}')$ (by @eq-IV3). Therefore, since $X_{i}'=(X_{i1}',Y_{i2}')$ are included in the equation for $Y_i$, there are no variations left in $Y_i$ that correlate with $Z_{i2}.$ Thus the instrumental variables $Z_{i2}$ have zero coefficients in the equation for $Y_i.$

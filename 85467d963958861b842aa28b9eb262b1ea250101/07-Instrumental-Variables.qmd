...
The **reduced form** model gives us the relationship between 

* the $(K_2\times 1)$ endogenous regressors $Y_{i2}$ and 
* the $(\ell\times 1)$ instruments $Z_{i}.$

The linear reduced form model for $Y_{i2}$ is given by 
$$
\begin{align}
Y_{i2} 
&= \Gamma'Z_i + u_{i2}\\ 
...
The $(\ell\times K_2)$ dimensional coefficient matrix is defined by the projection coefficient
$$
\Gamma = E(Z_iZ_i')^{-1}E(Z_iY_{i2}')
$${#eq-Gamma1}

The projection coefficient $\Gamma$ (@eq-Gamma1) is well defined and unique if @eq-IV2 holds.  

...
$$
\begin{align*}
E(X_{i1}\varepsilon_i)&=0\\[1.5ex]
E(Y_{i2}\varepsilon_i)&\neq 0\\[1.5ex]
E(Y_{i1}\varepsilon_i)&\neq 0\\
\end{align*}
$$
...
$${#eq-IV1}
$$
\operatorname{rank}\left(E(Z_iZ_i')\right)=\ell  
$${#eq-IV2}
$$
\operatorname{rank}\left(\underbrace{E(Z_iX_i')}_{(\ell\times K)}\right)=K, 
$${#eq-IV3}
...
* $Z_{i1}$: The *exogenous variables* 
* $Y_{i2}$: The *endogenous variables*
...
> What variables can be used as instruments? 

1. According to @eq-IV1, instrumental variables must be uncorrelated with the error term.
2. From the rank condition (@eq-IV3) it follows that the instrumental variables are correlated with the endogenous variables $Y_{i2}$ also after controlling for the other exogenous variables $Z_{i1}.$[^1] (Otherwise, the instrumental variables would not add any new information since they could be expressed as linear combinations of the already included exogenous variables $Z_{i1}.$)
...
These two requirement mean that the instrumental variables $Z_{i2}$

* are determined outside the systems for the endogenous dependent variable $Y_{i1}$ and the endogenous variables $Y_{i2}.$
...
When $X_{i}$ is a mis-measured version of $Z_i$ a common choice for
an instrument $Z_{i2}$ is an alternative measurement of $Z_i.$ 

For this $Z_{i2}$ to satisfy the property of an instrumental variable the measurement error in $Z_{i2}$ must be independent of that in $X_i.$

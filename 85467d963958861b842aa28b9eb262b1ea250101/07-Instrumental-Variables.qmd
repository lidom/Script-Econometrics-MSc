...
for(b in 1:B){
  ...
  V            <- rnorm(n, m = 1, sd = 0.5) # exogenous
...
The $\ell=3$ dimensional instrumental variables vector $Z_i$ is then determined by coll, we take the exogenous constant $1$, the exogenous regressor $X_{i2}$ and the instrument $Z_{i2}$
$$
Z_i = (1,X_{i2},Z_{i2})'.
$$
...
* Checking @eq-IV1 ($E(Z_i\varepsilon_i)=0$): 
...
* Checking @eq-IV2 ($\operatorname{rank}(E(Z_iZ_i'))=\ell=3$): 
...
=&\operatorname{rank}\left(
  \begin{matrix}
  1                    & E\left(X_{i2}\right)         & E\left(Z_{i2}\right) \\ 
  E\left(X_{i2}\right) & E\left(X_{i2}^2\right)       & E\left(X_{i2} Z_{i2}\right) \\ 
  E\left(Z_{i2}\right) & E\left(X_{i2} Z_{i2}\right)  & E\left(Z_{i2}^2\right) \\ 
  \end{matrix}\right)\overset{?}{=}3
\end{align*}
$$ 
This "no redundancies" condition is fulfilled since $Z_{i2}=V_i + u_i(\neq 1\neq X_{i2}\neq 0).$ A linear dependency would occur if $Z_{i2}$ 
  * if $Z_{i2}= 1$ (1st and 3rd column) 
  * if $Z_{i2}= X_{i2}$ (2nd and 3rd column) or 
  * if $Z_{i2}= 0$ (zero column).   
* Checking @eq-IV3 ($\operatorname{rank}(E(Z_iX_i'))=K=3$):  
$$
\begin{align*}
&\operatorname{rank}(E(Z_iX_i'))\\ 
=&\operatorname{rank}\left(
  \begin{matrix}
  1                    & E\left(X_{i2}\right)       & E\left(X_{i3}\right) \\ 
  E\left(X_{i2}\right) & E\left(X_{i2}^2\right)     & E\left(X_{i2} X_{i3}\right) \\ 
  E\left(Z_{i2}\right) & E\left(Z_{i2} X_{i2}\right)& E\left(Z_{i2} X_{i3}\right) \\ 
  \end{matrix}\right)
\end{align*}
$$   
Since $X_{i2}$ and $X_{i3}$ are independent, $E(X_{i2} X_{i3})=E(X_{i2})E(X_{i3})$ such that  
$$
\begin{align*}
&\operatorname{rank}(E(Z_iX_i'))\\ 
=&\operatorname{rank}\left(
  \begin{matrix}
  1                    & E\left(X_{i2}\right)       & \;\;1\;\cdot\;\; E\left(X_{i3}\right) \\ 
  E\left(X_{i2}\right) & E\left(X_{i2}^2\right)     & E\left(X_{i2}\right)E\left(X_{i3}\right) \\ 
  E\left(Z_{i2}\right) & E\left(Z_{i2} X_{i2}\right)& E\left(Z_{i2}\;\;\cdot\;\;X_{i3}\right) \\ 
  \end{matrix}\right)  
\end{align*}
$$ 
...
That is, the instrumental variable $Z_{i2}$ is a valid instrument. 
...
B              <- 10000    # Number of MC replications
beta_hat_TSLS  <- matrix(data = NA, # Container for 
                         nrow = 3,  # MC simulations
                         ncol = B)
...
for(b in 1:B){
  ...
  Y            <- cbind(1, X_2, X_3) %*% beta_true + eps
  ## Estimation
  beta_hat_TSLS[,b] <- coef(ivreg(Y ~ X_2 + X_3 | X_2 + V))
}

## MC means of \hat{\beta}_2 and \hat{\beta}_3 
mean_beta_hat_TSLS_2 <- round(mean(beta_hat_TSLS[2,]), 2)
mean_beta_hat_TSLS_3 <- round(mean(beta_hat_TSLS[3,]), 2)

## Unbiased estimator \hat{\beta}_2:
mean_beta_hat_TSLS_2  - beta_true[2]

## Unbiased estimator \hat{\beta}_3:
mean_beta_hat_TSLS_3  - beta_true[3]
```

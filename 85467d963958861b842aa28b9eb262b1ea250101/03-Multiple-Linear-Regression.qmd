...
$$
The latter can be shown as following. Observe that $\hat\beta$ consists of two random variables $X$ and $Y,$ and thus one needs to show first the conditional unbiasedness of $\hat\beta$ given $X$.  
$$
\begin{align*}
...
\end{align*}
$$
From this if follows, by the iterated law of expectations, that the OLS estimator is (unconditionally) unbiased:  
$$
\operatorname{Bias}(\hat\beta) = E\left(\operatorname{Bias}(\hat\beta|X)\right) = E(0) = 0.
$$


The famous Gauss-Markov Theorem states that the OLS estimator is the best (lowest variance) estimator of all linear (in $Y$) and unbiased estimators.

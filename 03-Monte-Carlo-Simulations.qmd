...
lines(density(estimates_mat[,1], bw = bw.SJ(estimates_mat[,1])), col="blue")
hist(estimates_mat[,2], main="n=15", xlab="", xlim = range(estimates_mat[,1]), prob = TRUE, ylim = c(0, 1.2))
lines(density(estimates_mat[,2], bw = bw.SJ(estimates_mat[,3])), col="blue")
...
* At least on average, the estimates $\bar{X}_n$ are close to the target parameter $\mu=10$. So their biases (see below) are close to zero.

* As the sample size increases, the distributions of the estimators $\bar{X}_n$  concentrate around the target parameter $\mu=10$. So the variances decrease as $n$ gets large.
